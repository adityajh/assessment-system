#!/usr/bin/env python3
"""
Execute SQL migration against Supabase via curl subprocess.
Uses curl to avoid Cloudflare's ASN block on urllib/requests.
"""
...
PAT        = "your_supabase_personal_access_token_here"
...
def run_sql(label: str, sql: str) -> bool:
    ...
    body   = output.replace(f"\nHTTP_STATUS:{status}", "").strip()
...
# ── Split into discrete statement groups ─────────────────────────────────────
# We maintain order and split by logical DDL section comments
# Strategy: split on CREATE TABLE, CREATE OR REPLACE VIEW, DO $$, INSERT INTO blocks
# We'll run tables individually, then views, then seed data
...
GROUP BY
    ...
    ("CREATE VIEW: v_peer_feedback_summary",   """
...
GROUP BY pf.recipient_id, s.canonical_name, pf.project_id, p.name, p.sequence;"""),
    ...
    ("CREATE VIEW: v_student_dashboard",       """
...
INSERT INTO students (student_number, canonical_name, aliases, is_active) VALUES
    ...
    (6,  'Archit Gupta',         ARRAY['Archit L', 'Archit Gupta ', 'Archit Gupta'],             TRUE),
    ...
    (16, 'Saumyaa Gupta',        ARRAY['Saumyaa M', 'Saumyaa Gupta ', 'Saumyaa Gupta'],         TRUE),
