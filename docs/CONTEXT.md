...
│  Read-only JSON data for external tools      │
└──────────────────────────────────────────────┘
```
...
To support cross-platform integrations (e.g., LMS platforms, external dashboards), the system provides a Next.js REST API layer (`/api/v1/*`).

- **Authentication:** External requests require a Bearer token (`Authorization: Bearer <TOKEN>`).
- **API Keys:** Tokens are generated by admins via the Settings UI (`/admin/settings/api-keys`). The database (`api_keys` table) stores only the **SHA-256 hash** of the token for security.
- **Endpoints:**
  - `GET /api/v1/students` - Active students and top-level metrics.
  - `GET /api/v1/students/[id]` - Full student profile (domain scores, peer feedback).
  - `GET /api/v1/projects` - Project dictionary.
