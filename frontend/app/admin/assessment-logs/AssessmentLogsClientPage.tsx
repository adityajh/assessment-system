                    ...
                    <tbody className="divide-y divide-slate-100">
                        {logs.map(log => (
                            <tr key={log.id} className="hover:bg-slate-50 transition-colors">
                                ...
                                <td className="py-3 px-6 whitespace-nowrap">{log.programs?.name || 'Unknown'} <span className="text-slate-400 mx-1">•</span> {log.term}</td>
                                <td className="py-3 px-6 whitespace-nowrap">{log.projects?.name || '-'}</td>
                                <td className="py-3 px-6 text-slate-500 truncate max-w-[200px]" title={log.file_name || ''}>{log.file_name || '-'}</td>
